...
-- ============================================================
-- Store original SendAsync refs before the patcher overwrites them
local originalSendAsync = {}
local function storeOriginals()
    local channels = TextChatService:FindFirstChild("TextChannels")
    if not channels then
        channels = TextChatService:WaitForChild("TextChannels", 8)
    end
    if not channels then return end
    for _, ch in ipairs(channels:GetChildren()) do
        if ch:IsA("TextChannel") then
            originalSendAsync[ch] = ch.SendAsync
        end
    end
...
local function sendToChat(msg)
    ...
    -- Always try RBXGeneral first specifically
    local function tryChannel(ch)
        if not ch then return false end
        local origFn = originalSendAsync[ch]
        if origFn then
            return pcall(function() origFn(ch, msg) end)
        end
        return pcall(function() ch:SendAsync(msg) end)
    end

    local channels = TextChatService:FindFirstChild("TextChannels")
    if channels then
        -- Priority: RBXGeneral → All → any non-private channel
        local general = channels:FindFirstChild("RBXGeneral")
        if general then ok = tryChannel(general) end

        if not ok then
            for _, ch in ipairs(channels:GetChildren()) do
                if ch:IsA("TextChannel") then
                    local name = ch.Name:lower()
                    -- Skip private/team/whisper channels
                    if not name:find("private") and not name:find("whisper")
                    and not name:find("team") and not name:find("direct") then
                        ok = tryChannel(ch)
...
end)    ["tbh"]="to be honest",["ngl"]="not going to lie",
    ["btw"]="by the way",["bty"]="by the way",["bytheway"]="by the way",
    ["fwiw"]="for what it's worth",["fyi"]="for your information",
    ["omg"]="oh my god",["omgg"]="oh my god",
    ["brb"]="be right back",["gtg"]="got to go",["afk"]="away from keyboard",
